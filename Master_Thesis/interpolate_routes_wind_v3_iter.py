...
sav_fol="/Volumes/LaCie/Master Thesis/Wind_parquet/"

###Frequency wind
wind_dates=pd.DataFrame(pd.date_range(start='2008/4/3', end='2009/1/1',inclusive="left",freq="6H"),columns=["timestamp"]).set_index("timestamp").index
...
xs=xa.open_dataset("/Volumes/LaCie/Master Thesis/Codes/Wind/1993/01/1993010100-IFR-L4-EWSB-BlendedWind-GLO-025-6H-REPv6-20181220T174324-fv1.0.nc")
x_grid=xs.lon.values
y_grid=xs.lat.values


##Starting date range iteration.  Up to November 2019, to avoid running out of Weather files
for timestart in pd.date_range(start="2008/4/3",end="2009/1/1",inclusive="left",freq="D"):
    
    part1_timestart=timestart
    
    per_route=[]
    ###Iterate per route (6 routes)
    for route_iter_name,group in exp_df.groupby("route_iter_name"):
        ...
        gr_points_fr_x_wind=(np.abs(x_grid-group["lon"].to_numpy().reshape(-1,1))).argmin(axis=1)[:,np.newaxis]
        
        gr_points_fr_y_wind=(np.abs(y_grid-group["lat"].to_numpy().reshape(-1,1))).argmin(axis=1)[:,np.newaxis]
        
        gr_points_to_x_wind=(np.abs(x_grid-group["lon_to"].to_numpy().reshape(-1,1))).argmin(axis=1)[:,np.newaxis]
        
        gr_points_to_y_wind=(np.abs(y_grid-group["lat_to"].to_numpy().reshape(-1,1))).argmin(axis=1)[:,np.newaxis]
        
        
        ###Concatenate all grids indeces in same row
        ##Dataframe w original data from group.
        hull_test_index=np.concatenate((gr_points_fr_x_wind,gr_points_fr_y_wind,gr_points_to_x_wind,gr_points_to_y_wind,
        ...
        hull_test_index=pd.DataFrame(hull_test_index,columns=["wind_box_fr_x","wind_box_fr_y","wind_box_to_x","wind_box_to_y","time_f_hours","lon_wp","lat_wp","heading_to","leg","eca"])
        ...
        hull_test=np.concatenate((gr_points_fr_x_wind,gr_points_fr_y_wind,gr_points_to_x_wind,gr_points_to_y_wind),axis=1)
        ...
        cell_index=pd.DataFrame(cell_index,columns=["longitude_xa","latitude_xa","wind_box_fr_x","wind_box_fr_y","wind_box_to_x","wind_box_to_y"])
        ...
        cell_index=pd.merge(cell_index,hull_test_index,on=["wind_box_fr_x","wind_box_fr_y","wind_box_to_x","wind_box_to_y"])
        
        cell_index=cell_index.assign(time_wind_xa=wind_dates[wind_dates.get_indexer(cell_index.time_xa,method="nearest")])
        
          
        cell_index=cell_index.assign(year_wa=cell_index.time_wind_xa.dt.year,
                                      month_wa=cell_index.time_wind_xa.dt.month,
                                      day_wa=cell_index.time_wind_xa.dt.day,
                                      hour_wa=cell_index.time_wind_xa.dt.hour)
            
            
        cell_index=cell_index.assign(file_name_wind=cell_index.apply(lambda x: glob.glob("/Volumes/LaCie/Master Thesis/Codes/Wind/{}/{:02d}/{}{:02d}{:02d}{:02d}*".\
                                                                          format(x.year_wa,x.month_wa,
                                                                                x.year_wa,x.month_wa,x.day_wa,x.hour_wa))[0],axis=1))
            ##Wind iteration
        per_day=[]
        ##Iterate over valid files only (relevant to the specific route)
        for day, group in cell_index.groupby("file_name_wind"):
            ...
            group=group.assign(eastward_wind=group.apply(lambda x: xs_in.sel(time=x.time_wind_xa,
                                                            lon=x.longitude_xa,lat=x.latitude_xa,method="pad").eastward_wind.values,axis=1),
                                northward_wind=group.apply(lambda x: xs_in.sel(time=x.time_wind_xa,
                                                          lon=x.longitude_xa,lat=x.latitude_xa,method="pad").northward_wind.values,axis=1))
            
            ###Every trace crossing within a time stamp is averaged. VECTOR AVERAGE.
            group=group.groupby(["time_wind_xa","time_xa",'wind_box_fr_x', 'wind_box_fr_y', 'wind_box_to_x', 
                                 'wind_box_to_y',"lon_wp","lat_wp","heading_to","leg","eca"]).agg({'eastward_wind':"mean",'northward_wind':"mean"}).reset_index()
            
            ###Speed and Direction . Speed in m/seconds then transformed to knots and direction in degrees . From 0 to 360 
            group=group.assign(wind_speed_kts=np.sqrt(group.eastward_wind**2+group.northward_wind**2)*1.94384449,
                                wind_dir=np.mod((np.arctan2(group.eastward_wind,group.northward_wind) * 180 / np.pi)+180,360))
            
            ###Every trace crossing within a time stamp is averaged.
            group=group.groupby(["time_wind_xa","time_xa",'wind_box_fr_x', 'wind_box_fr_y', 'wind_box_to_x', 'wind_box_to_y',
                                      "lon_wp","lat_wp","heading_to","leg","eca"]).agg({"wind_dir":"mean","wind_speed_kts":"mean"}).reset_index()
               
            group=group.assign(app_wind_dir=np.where((group.wind_dir-group.heading_to)>0,group.wind_dir-group.heading_to,360+(group.wind_dir-group.heading_to)))
        ...
        if group["leg"].iloc[0]==1:
            part1_timestart=per_day.loc[per_day.time_wind_xa.last_valid_index(),"time_wind_xa"]+timedelta(days=2)
            
    ##Concatenate all routes
    per_route_g=pd.concat(per_route).reset_index(drop=True)
    
    per_route_g=per_route_g.assign(app_wind_dir=np.where(per_route_g.app_wind_dir.between(0,45),1,
                                                        np.where(per_route_g.app_wind_dir.between(45,90),2,
                                                            np.where(per_route_g.app_wind_dir.between(90,135),3,
                                                                    np.where(per_route_g.app_wind_dir.between(135,180),4,
                                                                              np.where(per_route_g.app_wind_dir.between(180,225),5,
                                                                                      np.where(per_route_g.app_wind_dir.between(225,270),6,
                                                                                                np.where(per_route_g.app_wind_dir.between(270,315),7,
                                                                                                        np.where(per_route_g.app_wind_dir.between(315,360),8,None)))))))) )
    ...
    per_route_g.to_parquet(sav_fol+"voyage_started_{}".format(timestart.date()))
